...
//			NVIC_SystemReset();
		...
		else if((Rec_ID == BOOTLOADER_ID)&& (REC_DLC == 8))
		{
			for (i=0; i<8; i++) 
			{
				gSysParm.canRxBuf[i] = rx_message_struct.data[i];
...
void can_transmit_cycledata(uint8_t cyc)
{
  ...
  tx_message_struct.frame_type = CAN_TFT_DATA;
  tx_message_struct.dlc = 8;
  tx_message_struct.data[0] = 0x42;
  tx_message_struct.data[1] = 0x44;
  tx_message_struct.data[2] = 0x52;
  tx_message_struct.data[3] = cyc;

  transmit_mailbox = can_message_transmit(CAN1, &tx_message_struct);
  while(can_transmit_status_get(CAN1, (can_tx_mailbox_num_type)transmit_mailbox) != CAN_TX_STATUS_SUCCESSFUL);
}

void can_transmit_precompile()
{
  uint8_t transmit_mailbox;
  can_tx_message_type tx_message_struct;
  tx_message_struct.standard_id = gSysParm.basePara.CanDataId;
  ...
  tx_message_struct.frame_type = CAN_TFT_DATA;
  tx_message_struct.dlc = 8;
  tx_message_struct.data[0] = 0x53;
  tx_message_struct.data[1] = 0x4C;
  tx_message_struct.data[2] = 0x41;
  tx_message_struct.data[3] = 0X52;
