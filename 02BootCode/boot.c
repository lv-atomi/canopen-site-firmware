...
void app_jump_address_set(void)
{
	...
	dwStkPtr = *(uint32_t *)APP_ADDRESS;
	dwJumpAddr = *(uint32_t *)(APP_ADDRESS + sizeof(uint32_t));
...
void Init_Ram()
{
	...
	for(i = 0;i < DLCLENGTH;i++)
	{
		gSysParm.canRxBuf[i] = 0;
		gSysParm.canTxBuf[i] = 0;
	}
}
