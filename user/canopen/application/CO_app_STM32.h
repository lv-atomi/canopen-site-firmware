...
/* CANHandle : Pass in the CAN Handle to this function and it will be used for
...
typedef struct {
  uint8_t desiredNodeID; /* This is the Node ID that you ask the CANOpen
                          * stack to assign toggle your device, although it
                          * might not always be the final NodeID, after calling
                          * canopen_app_init() you should check ActiveNodeID of
...
#error fdcan seems not supported by at32?
  ...
  uint8_t outStatusLEDGreen; // This will be updated by the stack - Use them for
                             // the LED management
  uint8_t outStatusLEDRed;   // This will be updated by the stack - Use them for
                             // the LED management
  CO_t *canOpenStack;

} CANopenNodeSTM32;

// In order to use CANOpenSTM32, you'll have it have a canopenNodeSTM32
// structure somewhere in your codes, it is usually residing in CO_app_STM32.c
extern CANopenNodeSTM32 *canopenNodeSTM32;

/* This function will initialize the required CANOpen Stack objects, allocate
 * the memory and prepare stack for communication reset*/
int canopen_app_init(CANopenNodeSTM32 *canopenSTM32);
/* This function will reset the CAN communication periperhal and also the
 * CANOpen stack variables */
int canopen_app_resetCommunication();
/* This function will check the input buffers and any outstanding tasks that are
 * not time critical, this function should be called regurarly from your code
 * (i.e from your while(1))*/
