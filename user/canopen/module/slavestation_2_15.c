...
void motor_init(void) {
  ...
  gpio_init_struct.gpio_pins = GPIO_PINS_6 | GPIO_PINS_7; // PB6 and PB7
  ...
  prescaler_value = (uint16_t)(SystemCoreClock / 24000000) - 1;

  // TMR4 time base configuration for PWM
  tmr_base_init(TMR4, 20000, prescaler_value);
  ...
  tmr_channel_value_set(TMR4, TMR_SELECT_CHANNEL_1, 10000); // 50% Duty cycle
  tmr_output_channel_buffer_enable(TMR4, TMR_SELECT_CHANNEL_1, TRUE);

  tmr_period_buffer_enable(TMR4, TRUE);

  // TMR10 time base configuration for RPM sensing
  tmr_base_init(TMR10, 20000, prescaler_value);
  ...
  tmr_channel_value_set(TMR10, TMR_SELECT_CHANNEL_1, 10000); // 50% Duty cycle
...
static ODR_t my_OD_read_6100(OD_stream_t *stream, void *buf, OD_size_t count,
                             OD_size_t *countRead) {
  ...
  if (stream->subIndex == 0) {
    ...
    *countRead = sizeof(uint8_t);
    ...
    bool_t value = 123; // FIXME: Read your actual value here.
    CO_setUint8(buf, value);
    *countRead = sizeof(bool_t);
  } else {
    return ODR_SUB_NOT_EXIST; // Invalid subindex
  }
...
static ODR_t my_OD_read_6101(OD_stream_t *stream, void *buf, OD_size_t count,
                             OD_size_t *countRead) {
  ...
  case 2:
  case 3: {
    int32_t value = 123; // FIXME:Read your actual value here.
...
static ODR_t my_OD_write_6101(OD_stream_t *stream, const void *buf,
  ...
  case 4:
    return ODR_READONLY; // Subindex 4 is read-only
  default:
    return ODR_SUB_NOT_EXIST; // Invalid subindex
  }
