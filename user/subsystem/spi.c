...
void init_spi(SPIPort *devport){
  ...
  if (devport->init_type.master_slave_mode == SPI_MODE_SLAVE){
    ...
    init_gpio_output(&devport->clk, GPIO_OUTPUT_PUSH_PULL, GPIO_DRIVE_STRENGTH_STRONGER);
    if (devport->cs.port == NULL) 
      init_gpio_output(&devport->cs, GPIO_OUTPUT_PUSH_PULL, GPIO_DRIVE_STRENGTH_STRONGER);
  }
  
  if (devport->mosi.port == NULL) init_gpio_input(&devport->mosi, GPIO_PULL_UP, GPIO_DRIVE_STRENGTH_STRONGER);
  if (devport->miso.port == NULL) init_gpio_output(&devport->miso, GPIO_OUTPUT_PUSH_PULL, GPIO_DRIVE_STRENGTH_STRONGER);
